...
def push_to_trmnl(question_data: dict, formatted_date: str) -> None:
    ...
    if not webhook_uuid:
        print("ERROR: TRMNL_WEBHOOK_UUID environment variable is not set.")
        sys.exit(1)
    ...
    if resp.status_code in (200, 201, 202):
        ...
        print(f"ERROR: TRMNL returned {resp.status_code}: {resp.text}")
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        print(f"ERROR: Failed to parse Claude's response as JSON: {exc}")
        sys.exit(1)
